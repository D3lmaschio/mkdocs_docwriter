...
from utils import write_config as _write_config, read_config as _read_config, quote_nav as _quote_nav
...
def index(filename: str , where: str):
    ...
    current_list = _mkdocs_config['nav']
    ...
    for i, key in enumerate(keys):
        ...
        if found is not None:
            current_list = found if isinstance(found, list) else []
        else:
            is_last = i == len(keys) - 1
            if not is_last:
                # Cria o nível e adiciona o próprio index.md
                new_level = {
                    key: [
                        {key: f"{'/'.join(keys[:i+1])}/index.md"}
                    ]
                }
                current_list.append(new_level)
                current_list = new_level[key]
            else:
                # Último nível = arquivo real
                current_list.append({key: _os.path.basename(filename)})
